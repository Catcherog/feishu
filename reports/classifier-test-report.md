...
> **R4 独立复审结论（2026-07-18）**：`MVP_PASS_WITH_DEBT`，5 个 P0 已关闭，可进入 R5。详见 `docs/ai/tasks/TASK-003-R5-V11-FIELD-VALIDATION-PACKET.md`。
...
下表与 `node --test --test-reporter=spec tests/migration-classifier.test.js` 的真实 TAP 输出逐套件对齐（首轮报告中以"断言覆盖数"误填测试数，本轮按 GPT `TASK-003` P1 修正要求重写）：

| Suite | Tests | 覆盖内容 |
|---|---:|---|
| 1. classifier public surface | 1 | `classifyRecord` / `classifyBatch` / `parseBudget` / `buildAccountingSummary` 入口签名和稳定 reason code 优先级 |
| 2. classifyBatch — full fixture correctness | 2 | 对 `cases.json` 全量 fixture 逐条分类 + 与 `expected.json` 逐字段对账 |
| 3. every reason code has at least one independent case | 10 | 每个 reason code 至少在一条 fixture 中作为 `primary_reason_code` 出现 |
| 4. four entity types covered | 4 | customer / project / model / makeup 均在 fixtures 中出现 |
| 5. multi-reason conflicts, priority, sorting, dedup | 6 | 同时命中多原因的优先级、稳定排序、去重及不变式验证 |
| 6. duplicate candidate decisions (SAME_ENTITY, DISTINCT_ENTITY, UNRESOLVED) | 3 | `SAME_ENTITY` / `DISTINCT_ENTITY` / `UNRESOLVED` 三类决策 |
| 7. budget parsing variants (D-025, 含 P0-1) | 16 | 空预算 / `面议` / 合法范围 / 上下限 / 模糊预算 / 前缀符号形式（`<`/`≤`/`>`/`≥`）/ 反向形式（`3000<`/`5000≥`/`3000≤`/`5000>`）/ 冲突范围 / rule_version 保留 |
| 8. P0-2 regression: D-020 customer status inference | 3 | CUSTOMER_ALIAS_020/PROJECT_ALIAS_014/CUSTOMER_ALIAS_009 状态传播 |
| 9. P0-3 regression: valid V2 project statuses 已交付 / 已归档 | 2 | PROJECT_ALIAS_012/013 ELIGIBLE |
| 10. P0-4 regression: D-023 valid need summary | 2 | CUSTOMER_ALIAS_019 ELIGIBLE；CUSTOMER_ALIAS_003 仍 BLOCKED |
| 11. classifyRecord (single-record, no inter-record context) | 3 | 无 batch context 时 `CUSTOMER_UNRESOLVED` 不触发、`ORPHAN_PROJECT` 退化为 hint-assume |
| 12. buildAccountingSummary reconciliation | 4 | 每实体 bucket 存在 + MIGRATABLE + NEEDS_REVIEW + BLOCKED = source_total + 主原因总和 = source_total + overall 对账 |
| 13. deterministic output (same fixtures -> identical SHA256) | 2 | 连续两次相同 fixtures 输出 SHA256 一致；乱序输入 SHA256 一致 |
| **合计** | **58** | |
...
| 不变式 | 验证方式 | 结果 |
|---|---|---|
| 每条记录恰有一个 `primary_reason_code` | `buildAccountingSummary` 对缺失抛错；测试 12 验证 | ✓ |
| `secondary_reason_codes` 稳定排序且无重复 | `sortAndDedupReasonCodes` + 测试 5/13 验证 | ✓ |
| MIGRATABLE + NEEDS_REVIEW + BLOCKED = source_total | `buildAccountingSummary` 每实体校验 + 测试 12 | ✓ |
| 主原因计数总和 = source_total | `buildAccountingSummary` + 测试 12 | ✓ |
| 确定性（顺序无关） | 测试 13：乱序输入 SHA256 一致 | ✓ |
| 确定性（重跑一致） | 测试 13：连续两次 SHA256 一致 | ✓ |
| P0-1：前缀符号 `<`/`≤`/`>`/`≥` 必须在数字之前 | 测试套件 7（前缀符号 4 + 反向形式 4 = 8 个测试） | ✓ |
| P0-2：客户状态推断传播关联项目的任意未知状态 | 测试套件 8（3 个测试） | ✓ |
| P0-3：合法 V2 状态 `已交付`/`已归档` 不应误判为 STATUS_NEEDS_REVIEW | 测试套件 9（PROJECT_ALIAS_012/013） | ✓ |
| P0-4：`has_valid_need_summary=true` 可作为身份证据 | 测试套件 10（CUSTOMER_ALIAS_019 ELIGIBLE；003 仍 BLOCKED） | ✓ |
...
| R3 | INDEPENDENTLY_VERIFIED_PASS（2026-07-18 GPT 独立复审通过） |
| R4 | INDEPENDENTLY_VERIFIED_PASS（2026-07-18 GPT 独立复审结论 `MVP_PASS_WITH_DEBT`，5 P0 已关闭，P1 文档债已在本报告 Section 2 修正） |
| R5 | NOT_STARTED（即将由 `TASK-003-R5-V11-FIELD-VALIDATION-PACKET.md` 启动） |
| R6 | NOT_STARTED |
| MIGRATION_PILOT_001 | NOT_APPROVED |

R4 已独立复审通过（`MVP_PASS_WITH_DEBT`），P1 套件计数已按 `TASK-003` Task 1 修正与 TAP 输出对齐。R5 v1.1 字段验证即将启动，但本报告不自动推进 R5；R6 与 `MIGRATION_PILOT_001` 保持未启动。
