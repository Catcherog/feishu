...
> **R5 第三修复批次 main commit**：将在 main commit 后通过 `git rev-parse HEAD` 获取（pre-backfill，详见下方"R5 第三修复批次自引用策略"）
> **R5 第三修复批次 commits**：R5 third fix main commit（SHA 待回填）→ R5 third fix backfill commit（SHA 待回填，详见下方"R5 第三修复批次自引用策略"）
...
| `reports/phaseR5-v11-field-validation-gpt-audit-package.md`（本文件） | 修改（R5 second fix + R5 third fix） | `f373374` + `3df9fc5` + `82d9886` + `672ed78` + `8dcd9fd` + `13dee71` + R5 third fix main commit + R5 third fix backfill commit | R5 second fix main commit `8dcd9fd` 中的 pre-backfill blob = `a1ac52ce6bd6812fe5e8b1deddf92f35265025c8`；R5 third fix main commit 中的 pre-backfill blob 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 blob 由 `git rev-parse HEAD:reports/phaseR5-v11-field-validation-gpt-audit-package.md` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | R5 second fix main commit `8dcd9fd` 中的 pre-backfill SHA256 = `f2ffe3fe250847e3406852dcf8e0834f34c67a6024480188d80cf3d1b1f10512`；R5 third fix main commit 中的 pre-backfill SHA256 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 SHA256 由 `sha256sum` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | SELF_REPORTED |
| `docs/current-base-schema-export.json` | 修改（R5 third fix: V1 脱敏） | R5 third fix main commit（此文件在 backfill commit 中不修改，main == backfill） | 将在 R5 third fix backfill commit 中单列 | 将在 R5 third fix backfill commit 中单列 | REPRODUCIBLE_FROM_PUBLIC_REPO（公开 JSON，所有 field ID 已替换为 `<REDACTED_FIELD_ID>`，保留 name/type/description 统计价值） |
| `reports/phase1b-write-path-test-report.md` | 修改（R5 third fix: V1 脱敏） | R5 third fix main commit（此文件在 backfill commit 中不修改，main == backfill） | 将在 R5 third fix backfill commit 中单列 | 将在 R5 third fix backfill commit 中单列 | REPRODUCIBLE_FROM_PUBLIC_REPO |
| `docs/current-automation-audit.md` | 修改（R5 third fix: V1 脱敏） | R5 third fix main commit（此文件在 backfill commit 中不修改，main == backfill） | 将在 R5 third fix backfill commit 中单列 | 将在 R5 third fix backfill commit 中单列 | REPRODUCIBLE_FROM_PUBLIC_REPO |
| `scripts/verify_public_repo.py` | 修改（R5 third fix: 删除 S2_EXEMPT_FILES + phone_number 模式收紧为 hex-aware 边界） | R5 third fix main commit（此文件在 backfill commit 中不修改，main == backfill） | 将在 R5 third fix backfill commit 中单列 | 将在 R5 third fix backfill commit 中单列 | REPRODUCIBLE_FROM_PUBLIC_REPO |
| `tests/test_verify_public_repo.py` | 修改（R5 third fix: 运行时拼接合成 ID + 新增 9 条回归测试含 PhoneNumberBoundaryTests） | R5 third fix main commit（此文件在 backfill commit 中不修改，main == backfill） | 将在 R5 third fix backfill commit 中单列 | 将在 R5 third fix backfill commit 中单列 | REPRODUCIBLE_FROM_PUBLIC_REPO |
| `reports/v1.1-field-write-path-report.md` | 修改（R5 third fix: header 更新） | `f373374` + `3df9fc5` + `82d9886` + `672ed78` + `8dcd9fd` + `13dee71` + R5 third fix main commit + R5 third fix backfill commit | `b566061f85862725df3a7140f617d131a498e4aa`（R5 second fix main commit `8dcd9fd` 中的 pre-backfill blob）；R5 third fix main commit 中的 pre-backfill blob 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 blob 由 `git rev-parse HEAD:reports/v1.1-field-write-path-report.md` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | `096c5ab4284e51ad46eef8745d2c4b007eb0f567a0660ba8c369256c5ef1b573`（R5 second fix main commit `8dcd9fd` 中的 pre-backfill SHA256）；R5 third fix main commit 中的 pre-backfill SHA256 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 SHA256 由 `sha256sum` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | REPRODUCIBLE_FROM_PUBLIC_REPO（公开报告）+ SELF_REPORTED（其中 live 字段写读结果依赖私有脚本与真实 Base） |
| `PUBLIC_EXECUTION_ENTRYPOINT.md` | 修改（R5 third fix: 新增 Section 3.5） | `82d9886` + `672ed78` + `8dcd9fd` + `13dee71` + R5 third fix main commit + R5 third fix backfill commit | `cc8288074e4a39c29f3e073a97b77e63ac95a0fd`（R5 second fix main commit `8dcd9fd` 中的 pre-backfill blob）；R5 third fix main commit 中的 pre-backfill blob 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 blob 由 `git rev-parse HEAD:PUBLIC_EXECUTION_ENTRYPOINT.md` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | `af5de37bded38a43a69a881280f2105ce99944395e445d2355d719b351d00b8f`（R5 second fix main commit `8dcd9fd` 中的 pre-backfill SHA256）；R5 third fix main commit 中的 pre-backfill SHA256 将在 backfill commit 中单列；R5 third fix backfill commit 后的最终 SHA256 由 `sha256sum` 获取并由 GPT 复审时通过 Git 事实单列复核（不在本文件内自引用） | REPRODUCIBLE_FROM_PUBLIC_REPO |
...
1. R5 third fix main commit：审计包内本节使用 placeholder（"将在 R5 third fix backfill commit 中回填"）记录自引用。该 commit 中审计包的 pre-backfill blob 和 pre-backfill SHA256 将由 `git rev-parse HEAD:reports/phaseR5-v11-field-validation-gpt-audit-package.md` 和 `sha256sum` 在 main commit 后获取，并在 backfill commit 中回填到本节。
2. R5 third fix backfill commit：填入 R5 third fix main commit 中审计包的 pre-backfill blob/SHA256（将在第 5.1 节本文件行回填）。R5 third fix backfill commit 后的最终 blob 由 `git rev-parse HEAD:reports/phaseR5-v11-field-validation-gpt-audit-package.md` 获取，最终 SHA256 由 `sha256sum` 获取，二者由 GPT 在复审时通过 Git 事实单列复核，不在本文件内自引用。
3. 不得把 pre-backfill 与 post-backfill SHA 混写。
...
| **R5 第三修复批次 main commit SHA** | `git rev-parse HEAD`（R5 third fix main commit 后） | 0 | 将在 R5 third fix backfill commit 中回填 |
...
| AC9 | 公开仓库和 staged 安全扫描均为 `S0=0 S1=0 S2=0`；私有证据未被跟踪/暂存 | 满足（**误报**，扫描器有假阴性 bug） | **满足（经 R5 第三修复批次关闭）** | R5 主批次 + R5 第一修复批次：`verify_public_repo.py` tracked 140 files 误报 `S0=0 S1=0 S2=0`（扫描器有假阴性 bug）；R5 第二修复批次：tracked 146 files `S0=0 S1=0 S2=340`（340 个 S2 全部为 V1 阶段预存在暴露）；R5 第三修复批次：删除 S2_EXEMPT_FILES 豁免机制 + 三个 V1 文件脱敏（`docs/current-base-schema-export.json` 337 + `reports/phase1b-write-path-test-report.md` 2 + `docs/current-automation-audit.md` 1 全部 field ID 替换为 `<REDACTED_FIELD_ID>`），tracked 146 files **S0=0 S1=0 S2=0** + staged 5 files **S0=0 S1=0 S2=0**；私有文件在 gitignored 路径。**AC9 现在满足**：tracked 和 staged 均为 S0=0 S1=0 S2=0，无豁免机制，无 S2 暴露。 |
| AC10 | R5 审计包证据完整，工作树干净，提交已 push | **不满足**（R5 主批次：审计包有"见 git"占位、错误 HEAD、错误 commit 数、错误证据分级） | **满足（经 R5 第二修复批次关闭）** | R5 第一修复批次重写审计包，补齐所有 blob/SHA256、6 commit 链、正确证据分级、AC 逐条对齐；R5 第二修复批次进一步修正 stale backfill/HEAD/tracked-count/AC9/AC10 陈述，新增 R5 第二修复批次自引用策略；R5 第三修复批次更新审计包反映 S2_EXEMPT_FILES 删除 + V1 文件脱敏 + AC9 满足；R5 third fix commits + backfill 将在 push 后通过 `git rev-parse HEAD` 确认；SELF_REPORTED。 |
